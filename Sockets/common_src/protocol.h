...
protected:
    ...
    Protocol(Socket&& skt, const std::string& type_of_protocol):
            skt(std::move(skt)), type_of_protocol(type_of_protocol) {}
    ...
    std::string recv_some_string(size_t size) {
        Buffer buffer(size);
        int bytes_received = skt.recvsome(buffer.data(), size);
