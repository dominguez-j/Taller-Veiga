...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        s = connect(socket, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(socket, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
